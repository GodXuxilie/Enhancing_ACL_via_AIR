...
import time
from utils import AverageMeter, eval_adv_test, logger, eval_test, eval_adv_test_OOD, setup_seed
import numpy as np
import copy
...
parser.add_argument('--eval-ood-only', action='store_true',
                    help='if specified, eval the loaded model')
parser.add_argument('--eval-ood-only-AFF', action='store_true',
                    help='if specified, eval the loaded model')
...
parser.add_argument('--other_attack', type=str, default=None)
...
def main():
    ...
    if args.checkpoint != '':
        ...
        elif (not args.eval_only and not args.eval_ood_only and not args.eval_ood_only_AFF and not args.other_attack is not None) and not args.resume:
    ...
    if args.eval_only:
        model.eval()
        _, test_tacc = eval_test(model, device, test_loader, log, advFlag=None)
    ...
    if args.other_attack is not None:
        model.eval()
        t_loader = torch.utils.data.DataLoader(
            testset, batch_size=10000 if args.dataset != 'stl10' else 8000, shuffle=True, num_workers=0, pin_memory=True, drop_last=True)
        if args.other_attack == 'apgdce':
            runAA_APGDCE(model, t_loader, 'checkpoints/' + args.experiment + '/robustness_result_{}.txt'.format(args.other_attack))
        if args.other_attack == 'apgddlr':
            runAA_APGDDLR(model, t_loader, 'checkpoints/' + args.experiment + '/robustness_result_{}.txt'.format(args.other_attack))
        if args.other_attack == 'fab':
            runAA_FAB(model, t_loader, 'checkpoints/' + args.experiment + '/robustness_result_{}.txt'.format(args.other_attack))
        if args.other_attack == 'square':
            runAA_square(model, t_loader, 'checkpoints/' + args.experiment + '/robustness_result_{}.txt'.format(args.other_attack))
        return
    
    if args.eval_ood_only:
        model.eval()
        _, test_tacc = eval_test(model, device, test_loader, log)
        test_atacc = eval_adv_test_OOD(model, device, test_loader=args.dataset, epsilon=args.epsilon, alpha=args.step_size,
...
def runAA(model, loader, log_path):
    ...
    for images, labels in loader:
        images = images.cuda()
        labels = labels.cuda()
        adversary.run_standard_evaluation(images, labels, bs=512)

def runAA_APGDCE(model, loader, log_path):
    model.eval()
    adversary = AutoAttack(model, norm='Linf', eps=8/255, version='standard', log_path=log_path)
...
def runAA_APGDDLR(model, loader, log_path):
    model.eval()
    adversary = AutoAttack(model, norm='Linf', eps=8/255, version='standard', log_path=log_path)
...
def runAA_FAB(model, loader, log_path):
    model.eval()
    adversary = AutoAttack(model, norm='Linf', eps=8/255, version='standard', log_path=log_path)
...
def runAA_square(model, loader, log_path):
    model.eval()
    adversary = AutoAttack(model, norm='Linf', eps=8/255, version='standard', log_path=log_path)
